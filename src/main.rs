...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match &cli.command {
        ...
        Some(Commands::Install {}) => {
            let package_json = fs::PackageJson::read_package_json_saveable()?;

            let mut package_urls = Vec::new();
            let mut package_names = Vec::new();

            for key in package_json.dependencies.clone().unwrap().keys() {
                let response = request::get_package(key).await?;
                let dependencies = package_json.dependencies.clone().unwrap();
                let package_version = response
                    .versions
                    .get(dependencies.get(key).unwrap())
                    .unwrap();
                package_urls.push(package_version.dist.tarball.to_string());
                package_names.push(response.name);
            ...
            for key in package_json.dev_dependencies.clone().unwrap().keys() {
                let response = request::get_package(key).await?;
                let dependencies = package_json.dev_dependencies.clone().unwrap();
                let package_version = response
                    .versions
                    .get(dependencies.get(key).unwrap())
                    .unwrap();
